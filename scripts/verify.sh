...
check "P6.5 proxy-launcher script"       "ls \$HOME/.hermes/proxy/proxy-launcher.sh"
check "P6.5 proxy reachable :8318"       "curl -fsS --max-time 3 -o /dev/null -w '%{http_code}' http://127.0.0.1:8318/v1/models | grep -E '^[0-9]+$' >/dev/null"
check "P6.5 token watchdog LaunchAgent"  "ls \$HOME/Library/LaunchAgents/com.mejia.token-watchdog.plist"
check "P6.5 token watchdog script"       "ls \$HOME/.hermes/proxy/token-watchdog.sh"
check "P6.5 ~/.local/bin/claude symlink" "test -L \$HOME/.local/bin/claude"
check "P6.5 Anthropic OAuth credentials" "ls \$HOME/.claude/.credentials.json"
check "P6.5 long-lived token (preferred)" "grep -q '^CLAUDE_CODE_OAUTH_TOKEN=' \$HOME/.hermes/.env || true"
